...
import "../../../../investor/print.global.css";
...
export default function ViewIncentiveApplicationPage() {
  ...
  const handlePrint = async () => {
    ...
    const fileBase = application?.registrationNo
      ? `Application_${application.registrationNo}`
    ...
    try {
      // @ts-ignore - fonts may not exist on all browsers
  ...
  // Auto-open print dialog when ready if opened in new tab or `?auto=1`
  const ready = !!application && !isLoading && !flowLoading;
  ...
  useEffect(() => {
    ...
    if (shouldAuto) {
      // slight delay so layout/tooltip mounts settle
      const t = setTimeout(() => {
        handlePrint();
      }, 300);
      return () => clearTimeout(t);
    }
  }, [ready, autoParam]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // Add this near the top of your component (inside the component body)

  return (
    <div className="container-fluid card p-4">
      {/* Header (hidden in print) */}
      {/* PRINTABLE ROOT */}
      <div ref={printRef} id="print-root" className="print-page">
        {/* Print header (visible only on print) */}
        <div className="d-flex justify-content-between align-items-start mb-3 print-only">
          <div>
          ...
          {/* RIGHT SIDE: Printed-on + Print button horizontally aligned */}
          <div className="d-flex align-items-end gap-3">
            <div className="text-end">
              <div className="text-muted small">Printed on</div>
            ...
            <Button
              label="Print (PDF)"
              icon="pi pi-print"
              className="print-btn btn btn-outline-primary btn-sm rounded-5"
              onClick={handlePrint}
            />
          </div>
        </div>
        {/* Meta Card */}
        <Card className="mb-4 shadow-sm border-0 avoid-break">
        ...
        {/* Form Data */}
        <Card title="Application Information" className="shadow-sm avoid-break">
                ...
                <div className="row g-3">
                  {section.fields.map((field: any) => {
                    ...
                    if (isRepeaterField(field)) {
                      const rows: any[] = Array.isArray(rawValue)
                        ? rawValue
                        : [];
                              ...
                              <div className="table-responsive">
                                <table className="table table-sm table-bordered align-middle mb-0">
                                  ...
                                  <tbody>
                                    {rows.map((row, idx) => (
                                      <tr
                                        key={`${field.field_code}_row_${idx}`}
                                      >
                                        <td>{idx + 1}</td>
                                        {subFields.map((sf: any) => (
                                          <td
