  ...
  const { data: applications = [], isLoading } =
    useIncentiveApplicationSubmissions(
      selectedPolicyId && selectedSchemeCode
        ? { incentiveId: selectedSchemeCode, userId: Number(user?.id) }
        : { userId: Number(user?.id) }, // still pass userId even without scheme selected
    );
      ...
      <div className="card shadow-sm p-3 mb-4">
        <div className="row g-3">
          <div className="col-md-4">
            <label className="fw-semibold small">Policy</label>
            <Dropdown
              className="w-full border border-gray-300 rounded-md p-2 ps-3 focus:ring-2 focus:ring-[#e9090c] mt-3"
          ...
          <div className="col-md-4">
            <label className="fw-semibold small">Scheme</label>
            <Dropdown
              className="w-full border border-gray-300 rounded-md p-2 ps-3 focus:ring-2 focus:ring-[#e9090c] mt-3"
                  ...
                  <tbody>
                    {paginatedApplications.map((app: any, index: number) => (
                      <tr key={app.id}>
                        ...
                        <td>
                          <a
                            href={`/investor/incentive/print/${app.id}?auto=1`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="btn btn-outline-primary px-2 my-2 mx-auto"
                            style={{
                              padding: "0 2px",
                              fontSize: "0.9rem",
                              height: "20px",
                          ...
                          {app.applicationStatus === "REVERTED" && (
                            <a
                              href={`/investor/apply/${scheme.policy?.policy_code}/${scheme.scheme_code}/${scheme.version}`}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="btn btn-outline-primary px-2 mx-auto mt-2"
                              style={{
                                padding: "0 2px",
                                fontSize: "0.9rem",
                                height: "20px",
