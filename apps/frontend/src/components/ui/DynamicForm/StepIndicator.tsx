...
export default function StepIndicator({
    ...
    const getCircleClasses = (index: number, isCompleted: boolean, isCurrent: boolean, isClickable: boolean) => {
        const baseClasses =
            'w-9 h-9 me-3 rounded-pill flex items-center justify-center font-semibold text-sm transition-all duration-200 border-none shadow-none';
    ...
    const getLabelClasses = (isCompleted: boolean, isCurrent: boolean) => {
        const baseClasses = 'text-xs font-medium max-w-[140px] text-start leading-snug line-clamp-2';
                    ...
                    return (
                        <div key={step.id} className="flex w-100 flex-1 mb-2 flex-column">
                            ...
                            {index < steps.length - 1 && (
                                <div
                                    className={`flex-none h-[30px] w-[2px] ml-[18px] mt-2 rounded ${completedSteps.includes(index) ? 'bg-[#D3B3B3]' : 'bg-[#D3B3B3]'}`}
                                />
                            )}
                        </div>
                    );
