    ...
    try {
      const resp = await fetch(`${PY_API}/api/srs/insert-full`, {
        ...
        body: JSON.stringify({
          ...
          workflow_json:    genResult.workflow_json,
...
<div style={s.panel}>
  ...
  <div style={{
    ...
    {workflowJson && (
      ...
      >
        {showWorkflow ? (
          <><i className="bi bi-eye-slash" /> Hide Workflow</>
        ) : (
          <><i className="bi bi-eye" /> Show Workflow</>
        )}
      </button>
    )}
  </div>
  ...
  {!workflowJson && (
    <div style={{
      ...
      Click "Generate Workflow" to generate workflow steps
    </div>
  )}

  {/* 👉 SHOW ONLY WHEN BUTTON CLICKED */}
  {workflowJson && showWorkflow && (
    <>
      ...
      <div style={{
        ...
        <span>
          <i className="bi bi-list-ol" style={{ marginRight: 4 }} />
          {workflowJson.workflow_steps?.length ?? 0} workflow steps
        </span>

        <span>
          <i className="bi bi-file-earmark-text" style={{ marginRight: 4 }} />
          {workflowJson.officer_forms?.length ?? 0} officer forms
        </span>
      </div>

      {/* JSON TOGGLE */}
      <button
        style={{ ...s.btnSec, marginBottom: 10 }}
        onClick={() => setWorkflowExpanded(x => !x)}
      >
